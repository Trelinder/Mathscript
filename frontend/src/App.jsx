...
// Accept both the legacy sess_ format (existing stored sessions) and the new
// cryptographically random tok_ format (new sessions generated by this version).
const SESSION_ID_PATTERN = /^(sess_[a-z0-9]{6,20}|tok_[0-9a-f]{32})$/
const SCREEN_STORAGE_KEY = 'mathscript_screen'
const JWT_STORAGE_KEY = 'ms_jwt'
// Key used to hand the session ID to play.html via sessionStorage so it is
// never exposed in the URL, browser history, or server access logs.
const TYCOON_SESSION_KEY = 'ms_tycoon_session'

function createSessionId() {
  // Use the Web Crypto API (CSPRNG) instead of Math.random() so the token
  // is unpredictable.  The tok_ prefix is intentionally distinct from the
  // legacy sess_ pattern to prevent confusion with PHP session file names.
...
function App() {
  ...
  const handleStartTycoon = () => {
    if (typeof window !== 'undefined') {
      // Pass the session ID via sessionStorage (tab-local, not in URL/history/logs)
      // instead of the previous ?s= query-string approach.
      sessionStorage.setItem(TYCOON_SESSION_KEY, sessionId)
