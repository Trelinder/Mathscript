...
## Recent Changes
- 2026-02-27: Full Cursor branch merge (iffa + 1ff4) — applied all new frontend files: TeachingAnalogyCard.jsx component, motion.js (adaptive performance settings), locale.js (i18n number formatting), audio.js, audioPlayer.js utilities, updated observability.js. All component files updated: App.jsx (isAdminRoutePath), Quest.jsx (TeachingAnalogyCard display, fullAiRetrying state, heroLockMessage, quick mode retry button), Onboarding.jsx (useMotionSettings, realm selection UI), AnimatedScene.jsx (useMotionSettings for particle/animation scaling), MiniGame.jsx (updated). Backend: added ErrorPatcherMiddleware (structured error logging), _ADMIN_PATHS whitelist, honeypot response generator fixed to avoid catching legitimate /admin paths. Zenith hero images updated. All junk temp files (fetch_all_changes.js etc.) removed.
